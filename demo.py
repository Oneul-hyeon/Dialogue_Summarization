...
from src.evaluate import get_summary
import pickle
import random
...
def predict_summary(dialogue_number, dialogue) :
    ...
    return label, summary, llm_summary
...
with gr.Blocks() as demo:
    ...
    output_summary = gr.Textbox(label="KoBART 요약 결과")
    output_llm_summary = gr.Textbox(label="Local LLM요약 결과")
    # 버튼을 눌렀을 때 실행할 함수와 연결
    btn = gr.Button("요약")
    btn.click(fn=predict_summary, inputs=[input_text, output_dialogue], outputs=[gpt4o_summary, output_summary, output_llm_summary])

demo.launch(server_name="0.0.0.0", server_port=11029)